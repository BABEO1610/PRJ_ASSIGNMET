...
protected void doGet(HttpServletRequest request, HttpServletResponse response)
    ...
    if (action == null || action.equals("list")) {
        ...
        request.getRequestDispatcher("/Manager/employees.jsp").forward(request, response);
        
    } else if (action.equals("add")) {
        // ĐÚNG: Lấy danh sách chức vụ động từ DB
        List<Positions> listP = dao.getAllPositions();
        request.setAttribute("listP", listP);
        request.getRequestDispatcher("/Manager/employee-form.jsp").forward(request, response);
